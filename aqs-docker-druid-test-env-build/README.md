...
The 10 minute wait at the end of ingest.sh should probably provide the user a bit more status. And is there anything we can do to actually know when everything is done rather than assuming a time?

The ingest.sh script in general doesn't do a lot of error handling. It is only needed at build time, so maybe that's okay. But could/should it do better? 
